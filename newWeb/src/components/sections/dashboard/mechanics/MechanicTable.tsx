...
const MechanicTable = ({ searchText, rows, rowsPerPage = 4, loading }: MechanicTableProps): ReactElement => {
  ...
  const columns: GridColDef<Mechanic>[] = [
    {
      field: 'id',
      headerName: 'ID',
      resizable: false,
    ...
    },
    {
      field: 'name',
      headerName: 'Name',
      renderCell: (params: GridRenderCellParams<Mechanic, any, any, GridTreeNodeWithRender>) => (
      ...
      getActions: (params) => [
        <Tooltip title="Info">
          <GridActionsCellItem
            ...
            size="small"
            aria-label="View Mechanic details"
          />
        </Tooltip>,
        <Tooltip title="Edit">
          <GridActionsCellItem
  ...
  const handleOpenViewModal = useCallback((mechanic: Mechanic) => {
    setSelectedMechanic(mechanic);
    setViewModalOpen(true);
  }, []);

  const handleOpenEditModal = useCallback((mechanic: Mechanic) => {
    setSelectedMechanic(mechanic);
    setEditModalOpen(true);
  }, []);

  const handleOpenDeleteModal = useCallback((mechanic: Mechanic) => {
    setSelectedMechanic(mechanic);
  ...
  const handleSaveMechanic = useCallback(() => {
    console.log('Saving updated Mechanic');
    handleCloseModals();
  }, [handleCloseModals]);

  const handleDeleteMechanic = useCallback((mechanicId: number) => {
    console.log('Deleting Mechanic with ID:', mechanicId);
    handleCloseModals();
  }, [handleCloseModals]);
      ...
      {selectedMechanic && (
        <>
          ...
          <DeleteConfirmationModal
            ...
            onClose={handleCloseModals}
            onConfirm={() => handleDeleteMechanic(selectedMechanic.user.id)}
            mechanic={selectedMechanic}
          />
        </>
      )}
