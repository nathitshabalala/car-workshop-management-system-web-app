...
import { Mechanic, MechanicApi, PatchedMechanicRequest } from 'api';
...
const EditMechanicModal = ({ open, onClose, onSave, mechanic }: EditMechanicModalProps) => {
    const [editData, setEditData] = useState<PatchedMechanicRequest>({
        user: {
            name: mechanic.user.name || '',          
            surname: mechanic.user.surname || '',
            phone: mechanic.user.phone ?? '',        
            email: mechanic.user.email || '',
            street_address: mechanic.user.street_address || '',
            city: mechanic.user.city || '',
            postal_code: mechanic.user.postal_code || ''
        },
        password: ''
    });
    ...
    // Load mechanic data when the modal opens
    useEffect(() => {
        if (mechanic) {
            setEditData({
                user: {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Call the partial update API with updated mechanic details
            await mechanicApi.usersMechanicPartialUpdate(mechanic.user.id, dataToSend);
    
            onSave();
            setOpenSnackbar(true);
            onClose(); // Close the modal on success
        } catch (error) {
            setError('Failed to update mechanic details');
        } finally {
            setSubmitting(false);
        }
    };
